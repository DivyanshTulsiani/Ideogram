...
export const CommandMenu = (props: CommandProps) => {
  ...
  const listchat = props.chats.map(chat => <CommandItem className='flex gap-2  hover:bg-[#efefef] px-2  flex items-center w-full h-[2.5rem] rounded-lg' onSelect={() => {console.log("clicked", chat._id); fetchuserdataaNode(chat._id)}} key={chat._id}>
